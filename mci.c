...
/* ----- Port definitions ----- */
#define SOCKPORT	0 /*FIO4PIN1 Socket contact port (0 sempre aceita) */
#define SOCKINS		0x04		/* Card detect switch (bit2) */
#define	SOCKWP		0		/* Write protect switch (none) */
#define LEDR_ON()	FIO4CLR0 = LED8	/* Access indicator */
#define LEDR_OFF()	FIO4SET0 = LED8
...
#endif
/* Set MCICLK = MCLK_RW, power-save mode */
	MCI_CLOCK = (MCI_CLOCK & 0xF00) | 0x200 | (PCLK/MCLK_RW/2-1);
